...
import menue_icon from "../../../Chatgpt/ChatGpt Clong/src/assets/menu_icon.png";
import "./SideBar.css";
...
const SideBar = ({ field, setfield }) => {
    ...
    return (
        <div style={{ marginBottom: "1rem", backgroundColor: "#f0f4f9" }}>
            <div className="menu-icon-container" style={{ display: "flex", gap: "30px", marginBottom: "20px", alignItems: "center" }}>
                <img className="menu-icon" src={menue_icon} onClick={handleSideBar} alt="Menu Icon" />
                <span className="fs-5 fw-semibold">LearnHub</span>
            </div>

            <div className={`flex-shrink-0 p-3 sd ${showSideBar ? 'sidebar-show' : 'sidebar-hide'}`} style={{ marginTop: "50px", minHeight: "100vh" }}>
                <ul className="list-unstyled ps">
                        ...
                        {isEducationOpen && (
                            <div className="collapse show">
                                <ul className="btn-toggle-nav list-unstyled fw-normal pb-1 small">
                                    <li onClick={() => { setfield("Engineering") }
                                   }>
                                        <a href="#" className="link-body-emphasis d-inline-flex text-decoration-none rounded active">
                                           Engineering
                                        </a>
                                    </li>
                                    <li onClick={() => {setfield("Intermediate")}}>
                    ...
                    <li className="mb-1">
                        <button onClick={() => nav("/chat")} className="btn btn-toggle d-inline-flex align-items-center rounded border-0 collapsed">
                            Ask doubts?
                        </button>
                    </li>
                    <li className="mb-1">
                        <button onClick={() => nav("/task")} className="btn btn-toggle d-inline-flex align-items-center rounded border-0 collapsed">
                            Task Manager
                        </button>
                    </li>
                    <li className="border-top my-3" />
                    <li className="mb-1">
                        <button onClick={() => nav("/contact")} className="btn btn-toggle d-inline-flex align-items-center rounded border-0 collapsed">
                            Contact
                        </button>
                    </li>
                    <li className="mb-1">
                        <button onClick={handleOut} className="btn btn-toggle d-inline-flex align-items-center rounded border-0 collapsed">
                            Sign Out
                        </button>
                    </li>
                </ul>
